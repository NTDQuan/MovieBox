...
# Streamlit UI
title_html = f"""<h1 style="font-size: 40px; font-weight: bold; color: #B85042; font-family: 'Times New Roman', serif; text-shadow: 2px 2px 4px #000000;">Predicting Movie Success</h1>"""
...
with col1:
    name = st.text_input('Movie name ', help="Enter the movie name")
    budget = st.number_input('Budget ($)', format="%f", help="Enter the movie budget in dollars")
    runtime = st.number_input('Runtime (minutes)', format="%f", help="Enter the runtime of the movie in minutes")
...
with col2:
    ...
    director = st.text_input('Director', help="Enter the name of the director")
    company = st.text_input('Company', help="Enter the name of the production company")
    ...
    star = st.text_input('Star', help="Enter the name of the main star")
    writer = st.text_input('Writer', help="Enter the name of the writer")
...
if st.button('Get Result'):
    ...
    # Evaluate Director, Company, Genre, Star, Writer
    ...
    # Evaluate director, company, genre, etc.
    st.write(f'Director: {get_score_category(features_preprocessed["director_score"].iloc[0])}')
    st.write(f'Company: {get_score_category(features_preprocessed["company_score"].iloc[0])}')
    st.write(f'Genre: {get_score_category(features_preprocessed["genre_score"].iloc[0])}')
    st.write(f'Star: {get_score_category(features_preprocessed["star_score"].iloc[0])}')
    st.write(f'Writer: {get_score_category(features_preprocessed["writer_score"].iloc[0])}')
